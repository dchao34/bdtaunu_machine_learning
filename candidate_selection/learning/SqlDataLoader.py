...
class SqlDataLoader:
    ...
    def __init__(self,
        ...
        self.select_stmt = 'SELECT {0} FROM {1};'.format(columns, self.table_name)
        if debug:
            self.select_stmt = self.select_stmt.strip(';') + ' LIMIT 10;'

        # assemble update statement
        self.update_stmt_template = 'UPDATE ' + self.table_name + ' SET cand_score = %s WHERE eid = %s AND cidx = %s;'
    ...
    def update(self, record_id_arr, score_arr):
        ...
        update_list = [ (s, r[0], r[1]) for s, r in zip(score_list, record_id_list) ]
...
if __name__ == '__main__':
    ...
    database, table_name = 'testing', 'upsilon_candidates_sigmc'

    print 'Streaming over Candidates; not for machine learning: '
    print '---------------------------------------------------- '
    print
    with SqlDataLoader(database, table_name, arraysize=5,
